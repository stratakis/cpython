"""Generate trampoline_ehframe.c from a compiled perf trampoline object.

Copies the assembler-generated .eh_frame of the trampoline into a C file,
...
The build runs this under _bootstrap_python, which has no shared extension
modules, so it only uses pure Python modules and built-in ones.
"""
...
# Compiler macro that selects each slice's block in the generated C file.
_ELF_ARCH_MACROS = {
...
def write_source(entries: list[tuple[ObjectSlice, EhFrame]], output_path: str) -> None:
    """Write the C file; entries is a list of (ObjectSlice, EhFrame)."""
    if not entries:
    ...
    for obj_slice, eh_frame in sorted(entries, key=lambda e: e[0].arch_macro):
        lines += [
            ...
            "static const uint8_t trampoline_ehframe[] = {",
            _format_bytes(eh_frame.data),
            "};",
            "const _PyTrampolineEhFrame _Py_trampoline_ehframe = {",
            "    trampoline_ehframe,",
            "    sizeof(trampoline_ehframe),",
            f"    {eh_frame.fde_pc_offset},  /* fde_pc_offset */",
            f"    {eh_frame.fde_range_offset},  /* fde_range_offset */",
            f"    {eh_frame.field_size},  /* fde_field_size */",
            "};",
        ]
        keyword = "#elif"
    lines += [
        "#else",
        '#  error "trampoline_ehframe.c was not generated for this architecture"',
        "#endif",
        "",
    ]
    output = "\n".join(lines)
...
def generate(
    ...
    """Generate the C file from the given objects; return what was written."""
    ...
    write_source(entries, output_path)
    return entries


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate trampoline_ehframe.c from trampoline objects"
    )
    parser.add_argument(
        "objects",
        nargs="+",
    ...
    parser.add_argument(
        "-o", "--output", required=True, help="path of the C file to write"
    )
